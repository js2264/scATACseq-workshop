...
hemato$b_cells_annots <- ifelse(hemato$renamed_clusters %in% labels, hemato$?...?, NA)
p1 <- DimPlot(hemato, group.by = '?...?', reduction = "umap") + theme(aspect.ratio = 1) + scale_colour_discrete(na.value = '#aaaaaa')
DefaultAssay(hemato) <- 'SCT'
p2 <- FeaturePlot(hemato, features = ?...?, reduction = "umap") + 
        scale_colour_gradientn(
        ...
        theme(aspect.ratio = 1)
DefaultAssay(hemato) <- 'MOTIF'
p3 <- FeaturePlot(hemato, features = ?...?, reduction = "umap") + 
        scale_colour_gradientn(
            colors = c('#190886', '#6F07F8', '#F954A5', '#FF9D66', '#edf118'),
            limits = c(-5, 5), oob = scales::squish
        ) + 
